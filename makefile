# CODE GENERATED BY GEMINI 2.5

# 1. Add Variables
CC = gcc
CFLAGS = -Wall -g -Iinclude -std=c99 -D_POSIX_C_SOURCE=200809L -D_XOPEN_SOURCE=700 -Wextra -Werror -Wno-unused-parameter -fno-asm
TARGET = shell.out

# 2. Add .o files
OBJECTS = main.o parser.o tokeniser.o hop.o reveal.o jobs.o execute.o

# 3. Add Rules
# Default rule to build the program
all: $(TARGET)

# Rule to link all the object files into the final program
$(TARGET): $(OBJECTS)
	$(CC) $(CFLAGS) -o $(TARGET) $(OBJECTS)

# Compilation rules with updated dependencies:

# main.o 
main.o: src/main.c include/common.h include/parser.h include/tokeniser.h include/hop.h include/reveal.h include/execute.h include/jobs.h
	$(CC) $(CFLAGS) -c src/main.c

# parser.o
parser.o: src/parser.c include/parser.h include/tokeniser.h include/common.h
	$(CC) $(CFLAGS) -c src/parser.c

# tokeniser.o
tokeniser.o: src/tokeniser.c include/tokeniser.h include/common.h
	$(CC) $(CFLAGS) -c src/tokeniser.c

# hop.o
hop.o: src/hop.c include/hop.h
	$(CC) $(CFLAGS) -c src/hop.c

# reveal.o
reveal.o: src/reveal.c include/reveal.h
	$(CC) $(CFLAGS) -c src/reveal.c

#jobs.o
jobs.o: src/jobs.c include/jobs.h
	$(CC) $(CFLAGS) -c src/jobs.c

# execute.o
execute.o: src/execute.c include/execute.h
	$(CC) $(CFLAGS) -c src/execute.c


# 3. Cleanup
.PHONY: clean
clean:
	rm -f $(OBJECTS) $(TARGET)


# CODE GENERATED BY GEMINI 2.5